...
try:
    ...
    from PySide6.QtWidgets import (
        ...
        QMenu, QApplication, QStyle, QTreeWidgetItemIterator, QHeaderView
    )
    ...
    class TreePanel(QWidget):
        ...
        itemSelected = Signal(str, str)    # (path, type) - 點擊即發出
        ...
        def _add_item(self, parent: Optional[QTreeWidgetItem], gh_content, repo: GhRepository, branch: str) -> int:
            # 初始大小：如果是檔案則為檔案大小，目錄則預設為 0 (稍後累加)
            item_size = getattr(gh_content, 'size', 0)
            
            node = QTreeWidgetItem([gh_content.name, gh_content.type, str(item_size)])
            ...
            if gh_content.type == 'dir':
                ...
                try:
                    ...
                    for sub in subs:
                        # 遞迴累加子項目大小
                        total_size += self._add_item(node, sub, repo, branch)
                    
                    # 更新目錄顯示的大小
                    node.setText(2, str(total_size))
                    return total_size
                except Exception as e:
        ...
        def _on_item_clicked(self, item: QTreeWidgetItem, _col: int) -> None:
            ...
            path = data.get('path')
            typ = data.get('type')
            if path is not None and typ is not None:
                self.itemSelected.emit(path, typ)
