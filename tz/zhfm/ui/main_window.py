...
from PySide6.QtCore import Qt, QThreadPool, QUrl, QMimeData
...
import io, zipfile, re, os, urllib.parse, shutil, tempfile
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self._last_copy_temp_dir: str | None = None  # Path to temp dir of last internal copy
    ...
    def _init_ui(self) -> None:
        ...
        self.tree_panel.itemSelected.connect(self._on_item_selected)
        # 右鍵選單請求
        ...
        self.url_edit.setPlaceholderText("選取檔案後顯示 Raw 網址")
        ...
        web_url_label = QLabel("網頁網址：")
        self.web_url_edit = QLineEdit()
        self.web_url_edit.setReadOnly(True)
        self.web_url_edit.setPlaceholderText("選取檔案後顯示 GitHub 網頁網址")
        self.btn_copy_web_url = QPushButton("複製網址")
        self.btn_copy_web_url.clicked.connect(self._copy_web_url)

        self.btn_open_web_url = QPushButton("開啟網頁")
        self.btn_open_web_url.clicked.connect(self._open_web_url)

        web_url_layout.addWidget(web_url_label)
        web_url_layout.addWidget(self.web_url_edit)
        web_url_layout.addWidget(self.btn_copy_web_url)
        web_url_layout.addWidget(self.btn_open_web_url)

        # Pages 網址顯示列
        pages_url_layout = QHBoxLayout()
        pages_url_label = QLabel("Pages 網址：")
        self.pages_url_edit = QLineEdit()
        self.pages_url_edit.setReadOnly(True)
        self.pages_url_edit.setPlaceholderText("選取檔案後顯示 GitHub Pages 網址 (僅供參考)")
        self.btn_copy_pages_url = QPushButton("複製網址")
        self.btn_copy_pages_url.clicked.connect(self._copy_pages_url)

        self.btn_open_pages_url = QPushButton("開啟網頁")
        self.btn_open_pages_url.clicked.connect(self._open_pages_url)

        pages_url_layout.addWidget(pages_url_label)
        pages_url_layout.addWidget(self.pages_url_edit)
        pages_url_layout.addWidget(self.btn_copy_pages_url)
        pages_url_layout.addWidget(self.btn_open_pages_url)

        right_layout.addLayout(url_layout)
        right_layout.addLayout(web_url_layout)
        right_layout.addLayout(pages_url_layout)
    ...
    def _init_menu(self) -> None:
        ...
        act_new_repo = QAction("新增專案...", self)
        act_new_repo.triggered.connect(self._on_create_repo)
        m_file.addAction(act_new_repo)

        act_delete_repo = QAction("刪除專案...", self)
        act_delete_repo.triggered.connect(self._on_delete_repo)
        m_file.addAction(act_delete_repo)
        ...
        m_edit = menubar.addMenu("編輯(&E)")
        act_copy = QAction("複製", self)
        act_copy.setShortcut(QKeySequence.Copy)
        act_copy.triggered.connect(self._trigger_copy)
        m_edit.addAction(act_copy)

        act_paste = QAction("貼上", self)
        act_paste.setShortcut(QKeySequence.Paste)
        act_paste.triggered.connect(self._trigger_paste)
    ...
    def _on_create_repo(self) -> None:
        """顯示建立新 Repo 的對話框並執行建立動作"""
        if not self.current_user:
            QMessageBox.warning(self, "錯誤", "請先登入 GitHub。")
            return

        dialog = QDialog(self)
        dialog.setWindowTitle("建立新的 Repository")
        ...
        name_edit = QLineEdit()
        name_edit.setPlaceholderText("Repository 名稱 (必填)")
        
        desc_edit = QLineEdit()
        desc_edit.setPlaceholderText("描述 (選填)")
        
        private_chk = QCheckBox("設為私有 (Private)")
        private_chk.setChecked(True) # 預設私有比較安全
        
        readme_chk = QCheckBox("初始化 README.md")
        readme_chk.setChecked(True)
        
        form.addRow("名稱：", name_edit)
        form.addRow("描述：", desc_edit)
        form.addRow("", private_chk)
        form.addRow("", readme_chk)
        ...
        repo_name = name_edit.text().strip()
        if not repo_name:
            QMessageBox.warning(self, "錯誤", "Repo 名稱不能為空。")
            return
            
        repo_desc = desc_edit.text().strip()
        is_private = private_chk.isChecked()
        init_readme = readme_chk.isChecked()
        
        self.sb.showMessage(f"正在建立 Repo：{repo_name}...", 0)
        
        try:
            # 建立 Repo
            # auto_init=True 會自動建立 README，這樣 Repo 就不是空的，可以直接瀏覽
            new_repo = self.current_user.create_repo(
                name=repo_name,
            ...
            QMessageBox.information(self, "成功", f"成功建立 Repo：{new_repo.full_name}")
            
            # 重新整理 Repo 列表並選中新建的 Repo
            self.repo_panel.refresh()
            idx = self.repo_panel.repo_combo.findData(new_repo) # 需確認 RepoPanel 是否支援 findData 查找物件
            # 由於 Github 對象可能不同實例，findData 可能無效，我們改用名稱搜尋
            
            # 簡單搜尋
            for i in range(self.repo_panel.repo_combo.count()):
                r = self.repo_panel.repo_combo.itemData(i)
                if r and r.full_name == new_repo.full_name:
                    self.repo_panel.repo_combo.setCurrentIndex(i)
                    break
                    
        except GithubException as e:
            msg = e.data.get('message', str(e))
            if "Resource not accessible by personal access token" in msg:
                QMessageBox.critical(self, "權限不足", 
                    "無法建立 Repo：您的 Token 權限不足。\n"
                    "請確認 Token 已勾選 'repo' 權限 (Full control of private repositories)。"
                )
            else:
                QMessageBox.critical(self, "建立失敗", f"無法建立 Repo：\n{msg}")
            self.sb.showMessage("建立 Repo 失敗", 3000)
        except Exception as e:
            QMessageBox.critical(self, "錯誤", f"發生未預期的錯誤：{e}")
            self.sb.showMessage("建立 Repo 失敗", 3000)

    def _on_delete_repo(self) -> None:
        """刪除目前選取的 Repo，需輸入名稱確認"""
        repo = self.repo_panel.current_repo()
        if not repo:
            QMessageBox.warning(self, "錯誤", "請先選擇要刪除的專案 (Repo)。")
            return
        ...
        confirm_text, ok = QInputDialog.getText(
            ...
            "刪除專案確認", 
            f"警告：此動作無法復原！\n\n"
            f"您確定要刪除 {repo.full_name} 嗎？\n"
            f"若要確認，請輸入完整專案名稱 ({repo.full_name})："
        )

        if not ok:
            return

        if confirm_text.strip() != repo.full_name:
            QMessageBox.warning(self, "錯誤", "輸入的名稱不符，取消刪除。")
            return

        self.sb.showMessage(f"正在刪除 Repo：{repo.full_name}...", 0)
        
        try:
            repo.delete()
            QMessageBox.information(self, "成功", f"已刪除專案：{repo.full_name}")
            self.repo_panel.refresh()
            self.sb.showMessage("刪除成功", 3000)
        except GithubException as e:
            msg = e.data.get('message', str(e))
            if "Resource not accessible by personal access token" in msg:
                QMessageBox.critical(self, "權限不足", 
                    "無法刪除 Repo：您的 Token 權限不足。\n"
                    "刪除專案需要 'delete_repo' 權限，或者在 'repo' 權限下可能有額外限制。"
                )
            else:
                QMessageBox.critical(self, "刪除失敗", f"無法刪除 Repo：\n{msg}")
            self.sb.showMessage("刪除失敗", 3000)
        except Exception as e:
            QMessageBox.critical(self, "錯誤", f"發生未預期的錯誤：{e}")
            self.sb.showMessage("刪除失敗", 3000)
    ...
    def _trigger_copy(self):
        """從選單觸發複製 (針對檔案樹)"""
        # 如果焦點在文字編輯器或輸入框，讓系統預設處理 (不攔截)
        f = QApplication.focusWidget()
        if isinstance(f, (QLineEdit, QTextEdit)):
            # 嘗試呼叫 copy slot
            if hasattr(f, 'copy'):
                f.copy()
            return
        ...
        # 否則對檔案樹進行複製
        info = self.tree_panel.selected_info()
        if info:
            self._on_copy(info[0], info[1])
        else:
            self.sb.showMessage("請先選擇要複製的檔案", 2000)
    ...
    def _trigger_paste(self):
        ...
        target_dir = self.tree_panel.selected_dir_path()
        self._on_paste(target_dir)
    ...
    def _on_item_selected(self, path: str, typ: str):
        """當項目被點擊（單選）時，立即更新網址列"""
        repo = self.repo_panel.current_repo()
        ...
        # 1. 網頁網址 (Web URL) - HTML 頁面
        encoded_path = urllib.parse.quote(path)
        encoded_branch = urllib.parse.quote(branch)
        
        # GitHub 規範：檔案用 blob，目錄用 tree
        mode = "blob" if typ == "file" else "tree"
        
        # 根目錄特殊處理 (path 為空)
        if not path:
            web_url = f"{repo.html_url}/tree/{encoded_branch}"
        else:
            web_url = f"{repo.html_url}/{mode}/{encoded_branch}/{encoded_path}"
            
        self.web_url_edit.setText(web_url)

        # 2. Pages 網址 (GitHub Pages URL)
        # 預設格式: https://{owner}.github.io/{repo}/{path}
        # 注意: 這只是猜測，實際 URL 取決於使用者 Pages 設定 (是否有 custom domain, root folder 等)
        # repo.owner.login 取得擁有者名稱
        try:
            owner = repo.owner.login
            repo_name = repo.name
            
            # Pages URL 不需要 blob/tree/branch 結構，直接接路徑
            # 但需注意 Pages 只有特定 branch (如 main/gh-pages) 會生效，這裡僅單純串接路徑
            if not path:
                pages_url = f"https://{owner}.github.io/{repo_name}/"
            else:
                # Pages URL 路徑通常不需要 encode (瀏覽器會處理)，或是只需 encode 檔案名
                # 簡單起見先用 quote 處理路徑
                pages_path_parts = [urllib.parse.quote(p) for p in path.split('/')]
                pages_path_encoded = "/".join(pages_path_parts)
                pages_url = f"https://{owner}.github.io/{repo_name}/{pages_path_encoded}"
            
            self.pages_url_edit.setText(pages_url)
        except Exception:
            self.pages_url_edit.clear()

        # 3. 檔案網址 (Raw URL) - 原始檔案連結
        if typ == "file" and path:
            # Raw URL 需要逐層 encode 但保留斜線
            raw_path_parts = [urllib.parse.quote(p) for p in path.split('/')]
            raw_path_encoded = "/".join(raw_path_parts)
            # 使用 raw.githubusercontent.com
            # repo.full_name 是 "user/repo"
            raw_url = f"https://raw.githubusercontent.com/{repo.full_name}/{branch}/{raw_path_encoded}"
            self.url_edit.setText(raw_url)
        else:
            self.url_edit.clear()
    ...
    def _on_file_selected(self, path: str):
        ...
        # 1. 網頁網址 (Web URL) - Blob
        encoded_path = urllib.parse.quote(path)
        web_url = f"{repo.html_url}/blob/{encoded_branch}/{encoded_path}"
        self.web_url_edit.setText(web_url)
    ...
    def _copy_web_url(self) -> None:
        url = self.web_url_edit.text()
        if url:
            QApplication.clipboard().setText(url)
            self.sb.showMessage(f"網頁網址已複製到剪貼簿：{url}", 3000)
        else:
            self.sb.showMessage("目前沒有可複製的網址", 2000)
    ...
    def _open_web_url(self) -> None:
        ...
        if url:
            ...
            self.sb.showMessage(f"已在瀏覽器開啟網頁：{url}", 3000)
        else:
            self.sb.showMessage("目前沒有可開啟的網址", 2000)

    def _copy_pages_url(self) -> None:
        url = self.pages_url_edit.text()
        if url:
            QApplication.clipboard().setText(url)
            self.sb.showMessage(f"Pages 網址已複製到剪貼簿：{url}", 3000)
        else:
            self.sb.showMessage("目前沒有可複製的網址", 2000)

    def _open_pages_url(self) -> None:
        url = self.pages_url_edit.text()
        if url:
            QDesktopServices.openUrl(QUrl(url))
            self.sb.showMessage(f"已在瀏覽器開啟 Pages：{url}", 3000)
        else:
            self.sb.showMessage("目前沒有可開啟的網址", 2000)
    ...
    def _upload_local_files(self, upload_items: list[tuple[str, str]]) -> None:
        ...
        # 定義背景上傳任務
        def upload_task(items, repo_obj, branch_name, progress_callback=None):
            results = []
            print(f"Start uploading {len(items)} items...")
        ...
        # prog.setWindowModality(Qt.WindowModal) # 暫時移除強制回應以避免潛在的死鎖
        prog.show()
        ...
        def on_finished():
            ...
            # 重新載入前保存展開狀態
            expanded_paths = self.tree_panel.get_expanded_paths()
            self.tree_panel.load_root(repo, branch)
            # 載入後恢復展開狀態
            self.tree_panel.restore_expanded_paths(expanded_paths)
        ...
        def on_error(err):
            prog.close()
            exctype, value, tb = err
            print(f"Upload worker error: {value}\n{tb}")
    ...
    def _on_copy(self, path: str, typ: str) -> None:
        ...
        # 2. 準備外部剪貼簿 (Repo -> OS Explorer)
        # 建立暫存目錄
        temp_dir = os.path.join(tempfile.gettempdir(), 'zhfm_copy_buffer')
        if os.path.exists(temp_dir):
            try:
                shutil.rmtree(temp_dir)
            except Exception as e:
                print(f"清理暫存目錄失敗: {e}")
                temp_dir = os.path.join(tempfile.gettempdir(), f'zhfm_copy_{int(os.times().system)}')
        
        os.makedirs(temp_dir, exist_ok=True)
        self._last_copy_temp_dir = temp_dir
        ...
        files_to_download = [] # (repo_path, local_save_path)
        top_level_local_paths = []

        # 遍歷選取項目，建立目錄結構並收集檔案
        for p, t in items:
            name = os.path.basename(p)
            local_target = os.path.join(temp_dir, name)
            top_level_local_paths.append(local_target)
            
            if t == 'file':
                files_to_download.append((p, local_target))
            else:
                # 資料夾：先建立空目錄（確保空資料夾也能被複製）
                os.makedirs(local_target, exist_ok=True)
                # 遞迴找出所有檔案
                sub_files = self._gather_files_recursive(repo, branch, p)
                for sub_p in sub_files:
                    rel = os.path.relpath(sub_p, p)
                    sub_target = os.path.join(local_target, rel)
                    files_to_download.append((sub_p, sub_target))
        
        total_files = len(files_to_download)
        
        # 若有檔案則下載
        if total_files > 0:
            prog = QProgressDialog(f"正在下載 {total_files} 個檔案到剪貼簿...", "取消", 0, total_files, self)
            prog.setWindowModality(Qt.WindowModal)
            prog.show()

            for i, (r_path, l_path) in enumerate(files_to_download, 1):
                if prog.wasCanceled():
                    break
                try:
                    os.makedirs(os.path.dirname(l_path), exist_ok=True)
                    content = repo.get_contents(r_path, ref=branch)
                    with open(l_path, "wb") as f:
                        f.write(content.decoded_content)
                except Exception as e:
                    print(f"Copy download failed: {r_path}, {e}")
                prog.setValue(i)
            prog.close()

        # 設定系統剪貼簿
        md = QMimeData()
        urls = [QUrl.fromLocalFile(p) for p in top_level_local_paths if os.path.exists(p)]
        if urls:
            md.setUrls(urls)
            QApplication.clipboard().setMimeData(md)
            if len(items) == 1:
                self.sb.showMessage(f'已複製：{items[0][0]} (可貼上至檔案總管)', 3000)
            else:
                self.sb.showMessage(f'已複製 {len(items)} 個項目 (可貼上至檔案總管)', 3000)
        else:
             self.sb.showMessage('複製失敗：無法準備檔案', 3000)
    ...
    def _on_paste(self, target_dir: str) -> None:
        ...
        if md.hasUrls():
            for url in md.urls():
                if url.isLocalFile():
                    sys_paths.append(url.toLocalFile())
        
        # 2. 判斷是否為內部複製 (Repo -> Repo)
        # 條件：內部剪貼簿有東西，且系統剪貼簿的檔案路徑位於我們先前建立的暫存目錄中
        is_internal_paste = False
        if self._clipboard and sys_paths:
            # 取第一個檔案路徑來檢查是否在暫存目錄內
            first_path = sys_paths[0]
            if self._last_copy_temp_dir:
                try:
                    # 使用 commonpath 檢查路徑包含關係
                    # 注意：在 Windows 上路徑大小寫可能不一致，這裡做簡單檢查
                    if os.path.commonpath([first_path, self._last_copy_temp_dir]) == self._last_copy_temp_dir:
                        is_internal_paste = True
                except ValueError:
                    pass # 路徑位於不同磁碟機等情況

        if is_internal_paste:
            # --- 執行內部複製 (GitHub API) ---
            for (path, typ) in self._clipboard:
                if typ == 'file':
                    ...
                    try:
                        ...
                        try:
                            old = repo.get_contents(dst, ref=branch)
                            repo.update_file(dst, f'update {dst}', fobj.decoded_content, old.sha, branch=branch)
                        except GithubException:
        ...
        # 3. 處理外部檔案貼上 (檔案上傳)
        if sys_paths:
            if self._confirm("貼上上傳", f"偵測到 {len(sys_paths)} 個檔案/資料夾，確定要上傳到目前目錄？"):
                upload_items = []
                for p in sys_paths:
                    if os.path.isdir(p):
                        # 資料夾：遞迴並保留結構
