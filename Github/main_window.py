...
class MainWindow(QMainWindow):
    ...
    def _download_current_file(self) -> None:
        ...
        try:
            ...
            if content.type != "file":
                QMessageBox.warning(self, "提示", "目前選的是資料夾，請選檔案。")
                return
            data = content.decoded_content  # bytes
        ...
        try:
            with open(fname, "wb") as f:
                f.write(data)
            QMessageBox.information(self, "成功", "已下載到本機。")
        except Exception as e:
            QMessageBox.critical(self, "錯誤", f"寫入本機檔案失敗：{e}")
    ...
    def _upload_file(self) -> None:
        ...
        # 定義背景上傳任務
        def upload_task(paths, base, repo_obj, branch_name):
            total = len(paths)
            results = []
            for i, local_path in enumerate(paths, 1):
                fname = os.path.basename(local_path)
                repo_path = f"{base}/{fname}" if base else fname
                try:
                ...
                # 回報進度 (這裡簡化，直接在最後回傳所有結果，若要即時進度需改用 Signal)
            return results

        prog = QProgressDialog(f"正在上傳 {len(local_paths)} 個檔案...", "取消", 0, 0, self)
        prog.setWindowModality(Qt.WindowModal)
        prog.show()

        worker = Worker(upload_task, local_paths, base_dir, repo, branch)
        self._active_workers.append(worker)
        ...
        def on_result(results):
            ...
            QMessageBox.information(self, "上傳完成", msg)
    ...
    def _on_delete(self, path: str, typ: str) -> None:
        ...
        if not (repo and branch):
            return
        if typ == 'file':
            if not self._confirm('刪除檔案', f'確定要刪除檔案？\n{path}'):
                return
            try:
                file = repo.get_contents(path, ref=branch)
                repo.delete_file(path, f'delete {path}', file.sha, branch=branch)
            except GithubException as e:
                QMessageBox.critical(self, '錯誤', f'刪除失敗：{e}')
                return
        else:  # dir：遞迴刪除（逐檔刪）
            if not self._confirm('刪除資料夾', f'確定要遞迴刪除此資料夾及其內所有檔案？\n{path}'):
                return
            # 逐一刪除所有檔案（含子目錄 .gitkeep）
            files = self._gather_files_recursive(repo, branch, path)
            prog = QProgressDialog('正在刪除...', '取消', 0, len(files), self)
            prog.setWindowModality(Qt.WindowModal)
            prog.show()
            for i, fpath in enumerate(files, 1):
                if prog.wasCanceled():
                    QMessageBox.information(self, '已取消', '已取消刪除。')
                    return
                try:
                    fobj = repo.get_contents(fpath, ref=branch)
                    repo.delete_file(fpath, f'delete {fpath}', fobj.sha, branch=branch)
                except GithubException as e:
                    QMessageBox.warning(self, '警告', f'刪除失敗（略過）：{fpath}\n{e}')
                prog.setValue(i)
            prog.close()
            # 嘗試刪除資料夾下的 .gitkeep（若使用者有留下）
            try:
                gk = repo.get_contents(f'{path}/.gitkeep', ref=branch)
                repo.delete_file(f'{path}/.gitkeep', f'delete {path}/.gitkeep', gk.sha, branch=branch)
            except GithubException:
                pass
        self.tree_panel.load_root(repo, branch)
    ...
    def _on_copy(self, path: str, typ: str) -> None:
        # 只記錄目前選取項目（支援多次貼上）
        self._clipboard = [(path, typ)]
        self.sb.showMessage(f'已複製：{path}', 2000)

    def _on_paste(self, target_dir: str) -> None:
        repo = self.repo_panel.current_repo()
        branch = self.repo_panel.current_branch()
        if not (repo and branch) or not self._clipboard:
            return
    ...
    def _on_files_dropped(self, target_dir: str, paths: list[str]) -> None:
        ...
        if not files:
            return
        prog = QProgressDialog('正在拖曳上傳...', '取消', 0, len(files), self)
        prog.setWindowModality(Qt.WindowModal)
        prog.show()
        for i, local in enumerate(files, 1):
            if prog.wasCanceled():
                QMessageBox.information(self, '已取消', '已取消上傳。')
                return
            rel_name = os.path.basename(local)
            dst = f'{target_dir}/{rel_name}'.strip('/') if target_dir else rel_name
            try:
                with open(local, 'rb') as f:
                    data = f.read()
                try:
                    old = repo.get_contents(dst, ref=branch)
                    repo.update_file(dst, f'update {dst}', data, old.sha, branch=branch)
                except GithubException:
                    repo.create_file(dst, f'create {dst}', data, branch=branch)
            except Exception as e:
                QMessageBox.warning(self, '警告', f'上傳失敗（略過）：{local}\n{e}')
            prog.setValue(i)
        prog.close()
        self.tree_panel.load_root(repo, branch)
