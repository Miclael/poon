...
class MainWindow(QMainWindow):
    ...
    def _upload_local_files(self, local_paths: list[str], target_dir: str) -> None:
        """背景批次上傳本機檔案"""
        repo = self.repo_panel.current_repo()
        branch = self.repo_panel.current_branch()
        if not (repo and branch):
        ...
        worker = Worker(upload_task, local_paths, target_dir, repo, branch)
        self._active_workers.append(worker)
        ...
        def on_result(results):
            ...
            # QMessageBox.information(self, "上傳完成", msg) # 選擇不彈窗干擾，改用 Status Bar
    ...
    def _upload_file(self) -> None:
        ...
        base_dir = self.tree_panel.selected_dir_path()
        self._upload_local_files(local_paths, base_dir)
        ...
        if md.hasUrls():
            ...
            if paths:
                if self._confirm("貼上上傳", f"偵測到 {len(paths)} 個檔案/資料夾，確定要上傳到目前目錄？"):
                    # 若包含資料夾，需展開 (目前 _upload_local_files 僅支援檔案列表，需展開)
                    all_files = []
                    for p in paths:
                        if os.path.isdir(p):
                            ...
                            all_files.append(p)
                    self._upload_local_files(all_files, target_dir)
        elif md.hasImage():
            # 支援貼上截圖
            if self._confirm("貼上圖片", "偵測到圖片，確定要上傳為 pasted_image.png？"):
                ...
                img.save(tmp, "PNG")
                self._upload_local_files([tmp], target_dir)
